...
public class TabletScheduler extends LeaderDaemon {
    ...
    private void handleReplicaRelocating(TabletSchedCtx tabletCtx, AgentBatchTask batchTask)
        ...
        try {
            ...
            LOG.info(
                    ...
                    tabletCtx.getSrcReplica().getDataNodeId(), tabletCtx.getDestBackendId());
        } catch (SchedException e) {
            if (e.getStatus() == Status.SCHEDULE_RETRY) {
    ...
    private boolean deleteFromHighLoadBackend(TabletSchedCtx tabletCtx, List<Replica> replicas,
        ...
        for (Replica replica : replicas) {
            DataNodeLoadStatistic beStatistic = statistic.getBackendLoadStatistic(replica.getDataNodeId());
            if (beStatistic == null) {
                continue;
            }
